...
_RungeKuttaState = collections.namedtuple('_RungeKuttaState', 'y1, f1, integro, t0, t1, dt, interp_coeff')
...
# def integration(solution, K, t_, t, j, yi, t_1, t_2):
...
#     # points_interp = torch.tensor([t_1.item(),t_2.item()]).to(device)

#     dt_new = .01 #(t_[j]/100).item() #
#     points_to_interp = [torch.arange(0, t_[j].item(), dt_new).to(device)]
#     # points_to_interp = points_to_interp.to(device)
...
def integration(solution, K, points_to_interp, t, j, yi, t_1, t_2):
    # print('sdfsfdsfsf', solution.shape, K.shape)
    ...
    K_inv = torch.flip(K, dims=(0,))#.clone()
    K_inter = RegularGridInterpolator([t], K_inv.flatten()) ####should be inversed ????????
    K_new = K_inter(points_interp)
    K_new = torch.flip(K_new, dims=(0,))
    # print('fffasdf', torch.sum(K_new).shape, yi.shape)
    # print(t[:j].shape, I.shape)
    # I_inter = RegularGridInterpolator([t_[:j]], I.flatten())
    # I_new = I_inter(points_to_interp)
    # print('asdfasdfasdf')
    ...
    integro = torch.sum(K_new)/2 * yi * dt
    # print('safdasfasfd', integro)
    # integro = torch.sum(integro)*dt_new
    # print(integro)
...
def _runge_kutta_step(func, y0, f0, integro0, t0, dt, t1, solution, K, t, j, tableau):
    ...
    for i, (alpha_i, beta_i) in enumerate(zip(tableau.alpha, tableau.beta)):
        ...
        t_2 = ti
        # print('asdfdsf', t_2)
        # points_to_interp = torch.tensor([t_1.item(),t_2.item()])
        # print('integro0', integro0.shape)
        integro0 += integration(solution, K, t_, t, j, yi, t_1, t_2)
        print(integro0)
        # print('integro', integro0)
        t_1 = ti
        
        f = func(ti, yi, integro0, perturb=perturb)
    ...
    return y1, f1, integro1, y1_error, k
...
class RKAdaptiveStepsizeODESolver(AdaptiveStepsizeEventODESolver):
    ...
    def _before_integrate(self, t):
        t0 = t[0]
        # integro_0 = self.y0[:,1]*self.K[-1]*(t[1]-t[0])
        # # print('kkkk', integro_0)
        # f0 = self.func(t[0], self.y0, integro_0)
        # # print(f0)
        # if self.first_step is None:
        #     first_step = _select_initial_step(self.func, t, self.y0, self.K, self.order - 1, self.rtol, self.atol,
        #                                       self.norm, f0=f0)
        #     # print('first_step', first_step)
        # else:
        #     first_step = self.first_step
        first_step = self.first_step
        integro_0 = self.y0[:,1]*self.K[-1]*first_step
        f0 = self.func(t[0], self.y0, integro_0)

        self.rk_state = _RungeKuttaState(self.y0, f0, integro_0, t[0], t[0], first_step, [self.y0] * 5)
    ...
    def _adaptive_step(self, rk_state, solution, K, t, j):
        ...
        y0, f0, integro0, _, t0, dt, interp_coeff = rk_state
        ...
        y1, f1, integro1, y1_error, k = _runge_kutta_step(self.func, y0, f0, integro0, t0, dt, t1, solution, K, t, j, tableau=self.tableau)
        # dtypes:
        ...
        if accept_step:
            ...
            if on_jump_t:
                ...
                # we're now on.
                f1 = self.func(t_next, y_next, integro_next, perturb=Perturb.NEXT)
            f_next = f1
        else:
        ...
        rk_state = _RungeKuttaState(y_next, f_next, integro_next, t0, t_next, dt_next, interp_coeff)
        return rk_state
